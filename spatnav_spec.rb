...
describe 'Spatnav' do
  ...
  describe '#up' do
    it 'selects the correct links when navigating up' do
      browser.spatnav.up
      window.eval_js("document.activeElement.text;").to_s.should == 'C1'
      browser.spatnav.up
      window.eval_js("document.activeElement.text;").to_s.should == 'C2'
      browser.spatnav.up
      window.eval_js("document.activeElement.text;").to_s.should == 'C3'
    end
  end
  ...
  describe '#left' do
    it 'selects the correct links when navigating left' do
      ...
      window.eval_js("document.activeElement.text;").to_s.should == 'C1'
      browser.spatnav.left
      window.eval_js("document.activeElement.text;").to_s.should == 'B1'
      browser.spatnav.left
      window.eval_js("document.activeElement.text;").to_s.should == 'A1'
    end
  end
